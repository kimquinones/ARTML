...
# Create a Document-Feature Matrix (DFM) for the tokens
dfm_news <- tokens %>%
  dfm() %>%
  dfm_group(groups = docvars(tokens, "Month"))  # Grouping by 'Month'

# Calculate word frequencies for each month
word_freq <- textstat_frequency(dfm_news)

# Add relative frequency (RF)
word_freq <- word_freq %>%
  mutate(RF = (frequency / sum(frequency)) * 1000000)

# Create a function to perform keyness analysis and display results
...
# Generate keyness tables for each month and display them
for (month in months) {
